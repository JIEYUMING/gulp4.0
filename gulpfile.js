...
-------------------------------------------------------------- */
	...
	gulp.task('sass', function () {
		return gulp.src( [srcPath.css+'/*']) // 指明源文件路径、并进行文件匹配（编译风格：简洁格式）
			.pipe(sass())
...
-------------------------------------------------------------- */
	...
	gulp.task('clean', function() {
		return gulp.src([destPath.html+'/*']) //	清除所有
			.pipe(clean());
	});
	// 样式处理
	gulp.task('sassRelease', function () {
		return gulp.src( [srcPath.css+'/*']) // 指明源文件路径、并进行文件匹配（编译风格：压缩）
			.pipe(sass())
			.on('error', function (err) {
				console.error('Error!', err.message); // 显示错误信息
			})
	...
	gulp.task('release', gulp.series('clean','html',gulp.parallel('sassRelease','scriptRelease','images')));
/* = 帮助提示( Help )
-------------------------------------------------------------- */
